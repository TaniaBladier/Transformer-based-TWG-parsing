...
def main():
	...
	if len(argv) == 2 and argv[1] in ('-v', '--version'):
		from discodop_n import __version__
		print(__version__)
		...
		# help on subcommand
		execlp('man', 'man', 'discodop_n-%s' % argv[1])
	else:
		cmd = argv[1]
		...
		if cmd in globals():
			...
			getattr(__import__('discodop_n.%s' % cmd,
					fromlist=['main']), 'main')()


def treedraw():
	"""Usage: discodop_n treedraw [<treebank>...] [options]
...
def runexp(args=None):
	"""Usage: discodop_n runexp <parameter file> [--rerun]
...
Usage: discodop_n treetransforms [input [output]] [options]
...
Usage: discodop_n grammar <type> <input> <output> [options]
or: discodop_n grammar param <parameter-file> <output-directory>
or: discodop_n grammar info <rules-file>
or: discodop_n grammar merge (rules|lexicon|fragments) \
